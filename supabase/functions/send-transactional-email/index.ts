...
const SITE_NAME = "temptic"
// SENDER_DOMAIN is the verified sender subdomain FQDN (e.g., "notify.example.com").
// It MUST match the subdomain delegated to Lovable's nameservers — never the root domain.
// The email API looks up this exact domain; a mismatch causes "No email domain record found".
const SENDER_DOMAIN = "notify.temptic.com"
// FROM_DOMAIN is the domain shown in the From: header (e.g., "example.com").
// When display_from_root is enabled, this can be the root domain for cleaner branding,
// even though actual sending uses the subdomain above.
const FROM_DOMAIN = "notify.temptic.com"
...
// SHA-256 hex; only the hash of the unsubscribe token is ever persisted.
...
Deno.serve(withSentry("send-transactional-email", async (req) => {
  ...
  // 3. Issue a fresh unsubscribe token. We persist ONLY its SHA-256 hash; the
  //    plaintext token leaves this function in the email link and is never recoverable
  //    from the database.
  ...
  if (tokenError) {
    console.error('Failed to store unsubscribe token hash', { error: tokenError })
    await supabase.from('email_send_log').insert({
      message_id: messageId,
      template_name: templateName,
      recipient_email: effectiveRecipient,
      status: 'failed',
      error_message: 'Failed to store unsubscribe token',
    })
    return new Response(
